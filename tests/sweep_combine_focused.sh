#!/bin/bash
# sweep_combine_focused.sh — Focused sweep of combine kernel parameters.
set -euo pipefail
...
for CHUNK_SIZE in 32 64; do
    if (( CHUNK_SIZE % GROUP_SIZE != 0 )); then
        continue
    fi

    # Increment port to avoid conflicts from stale processes
    PORT=$((PORT + 1))
    ...
    echo "=== SMS=$SMS_COMBINE BATCH=$BATCH_SIZE GROUP=$GROUP_SIZE CHUNK=$CHUNK_SIZE PORT=$PORT ===" >&2
